...
//
// The external memory is one or two 23LC1024-I/P SRAM chips.  
// The byte address within each chip is 17 bits (bit16..bit0),
// with any higher address bits being ignored.
// We use address-bit 17 to select the second chip,
// PA7 selecting chip 0 and PA2 selecting chip 1. 
//
...
uint8_t spi0_exchange(uint8_t b1)
{
    ...
    if (SPI0.INTFLAGS & SPI_WRCOL_bm) {
        // Clear the wreckage from a write collision, if there was one.
        b2 = SPI0.DATA;
    }
    SPI0.DATA = b1;
    while (!(SPI0.INTFLAGS & SPI_IF_bm)) { /* [TODO] clear WDT if needed */ }
    b2 = SPI0.DATA; // This should have cleared SPI_IF flag.
    return b2;
}
...
// Return bits 0 and 1 set to indicate presence of SRAM chips.
{
    ...
    if (b1 == 0xa && b2 == 0x5) {
        // We have read back the correct values,
        // So we assume the the second chip is present.
        chips |= 0x02;
    }
    return chips;
}
...
// addr : starting byte address within the set of SRAM chips
{
    uint8_t b1, b2;
